...
public class InteractiveKLineView extends View {
    ...
    private int lastTradeInfoSize = 0; // 上一次的 entry 列表大小，用于判断是否成功加载了数据
    ...
    private OnLoadingData mLoadingData;
    ...
    public void setTradeInfoSet(TradeInfoSet set) {
        entrySet = set;
    }

    public void setOnLoadingData(OnLoadingData onLoadingData) {
        this.mLoadingData = onLoadingData;
    }
    ...
    public void notifyDataSetChanged(boolean invalidate) {
        ...
        render.setTradeInfoSet(entrySet);
    ...
    private final GestureDetector gestureDetector = new GestureDetector(getContext(), new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            if (!onLongPress && !onDoubleFingerPress && !onVerticalMove) {
                if (onDragging && !render.canScroll(distanceX) && render.canDragging(distanceX)) {
                    ...
                    if (DEBUG) {
                        ToolLog.d(TAG, "onScroll", "##d dragging: -------> " + distanceX + ", maxScrollOffset = "
                    ...
                    if (DEBUG) {
                        ToolLog.d(TAG, "onScroll", "##d scroll: ------->  " + distanceX + ", maxScrollOffset = "
                                + render.getMaxScrollOffset() + ", tranX = " + render.getCurrentTransX());
                    }
                }
                return true;
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            if (!onLongPress && !onDoubleFingerPress && !onVerticalMove && render.canScroll(0)) {
                if (DEBUG) {
                    ToolLog.d(TAG, "onFling", "##d onFling: ------->");
                }
    ...
    private void highlight(float x, float y) {
        ...
        int highlightIndex = render.getTradeInfoSet().getHighlightIndex();
        TradeInfo entry = render.getTradeInfoSet().getHighlightTradeInfo();
    ...
    private void dragging(float dx) {
        ToolLog.d("dx:" + dx + "  scrollOffset:" + render.getMaxScrollOffset());
        if (render.getMaxScrollOffset() < 0 || dx < 0) {
            if (null != mLoadingData && !mIsLoading) {
                if (dx < -20) {
                    mIsLoading = true;
                    mLoadingData.OnHistoryLoadingMore();
                } else if (dx > 20) {
                    mIsLoading = true;
                    mLoadingData.OnNewLoadingMore();
                }
            }
    ...
    private void springBack(float dx) {
        if (entrySet.getTradeInfoList().size() > lastTradeInfoSize) {
            scroll(dx);
        } else {
            releaseBack(dx);
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (DEBUG) {
            ToolLog.i(TAG, "onSizeChanged", "##d onSizeChanged: " + viewRect);
        }

        // 在 Android Studio 预览模式下，添加一些测试数据，可以把 K 线图预览出来
        if (isInEditMode()) {
//            TradeInfoSet entrySet = StockDataTest.parseKLineData(StockDataTest.KLINE);
//            if (entrySet != null) {
//                entrySet.computeStockIndex();
//            }

            setTradeInfoSet(entrySet);
        }
        if (entrySet == null) {
            entrySet = new TradeInfoSet();
        }
    ...
    public void computeScroll() {
        ...
        if (scroller.computeScrollOffset()) {
            ...
            if (DEBUG) {
                ToolLog.i(TAG, "computeScroll", "##d computeScrollOffset: canScroll = " + render.canScroll(dx)
            ...
            if (DEBUG) {
                ToolLog.d(TAG, "computeScroll", "##d overScrollOffset: canScroll = " + render.canScroll(0)
            ...
            if (!onTouch && overScrollOffset != 0 && kLineStatus == KLINE_STATUS_IDLE) {
                ...
                if (DEBUG) {
                    ToolLog.d(TAG, "computeScroll", "##d startScroll: LOADING... dx = " + dx);
                }
                ...
                if (kLineHandler != null) {
                    lastTradeInfoSize = entrySet.getTradeInfoList().size();
                    if (lastScrollDx > 0) {
    ...
    public void refreshComplete(boolean reverse) {
        ...
        if (DEBUG) {
            ToolLog.i(TAG, "refreshComplete", "##d refreshComplete: refreshComplete... overScrollOffset = " + overScrollOffset);
        }
