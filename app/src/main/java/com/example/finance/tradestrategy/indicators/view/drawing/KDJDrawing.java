...
public class KDJDrawing implements IDrawing {
    ...
    public void computePoint(int minIndex, int maxIndex, int currentIndex) {
        ...
        final TradeInfo.KDJ entry = entrySet.getTradeInfoList().get(currentIndex).getKdj();
        final int i = currentIndex - minIndex;

        if (currentIndex < maxIndex - 1) {
            final TradeInfo.KDJ entryNext = entrySet.getTradeInfoList().get(currentIndex + 1).getKdj();
            ...
            kBuffer[i * 4 + 2] = 0;
            kBuffer[i * 4 + 3] = entryNext.getK();

            dBuffer[i * 4 + 0] = 0;
            dBuffer[i * 4 + 1] = entry.getD();
            dBuffer[i * 4 + 2] = 0;
            dBuffer[i * 4 + 3] = entryNext.getD();

            jBuffer[i * 4 + 0] = 0;
            jBuffer[i * 4 + 1] = entry.getJ();
            jBuffer[i * 4 + 2] = 0;
            jBuffer[i * 4 + 3] = entryNext.getJ();
        }
    }
