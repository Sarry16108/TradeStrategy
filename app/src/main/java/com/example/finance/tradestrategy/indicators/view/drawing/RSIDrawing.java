...
public class RSIDrawing implements IDrawing {
    ...
    public void computePoint(int minIndex, int maxIndex, int currentIndex) {
        ...
        final TradeInfo.RSI entry = entrySet.getTradeInfoList().get(currentIndex).getRsi();
        final int i = currentIndex - minIndex;

        if (currentIndex < maxIndex - 1) {
            final TradeInfo.RSI entryNext = entrySet.getTradeInfoList().get(currentIndex + 1).getRsi();
            ...
            r1Buffer[i * 4 + 2] = 0;
            r1Buffer[i * 4 + 3] = entryNext.getRsi1();

            r2Buffer[i * 4 + 0] = 0;
            r2Buffer[i * 4 + 1] = entry.getRsi2();
            r2Buffer[i * 4 + 2] = 0;
            r2Buffer[i * 4 + 3] = entryNext.getRsi2();

            r3Buffer[i * 4 + 0] = 0;
            r3Buffer[i * 4 + 1] = entry.getRsi3();
            r3Buffer[i * 4 + 2] = 0;
            r3Buffer[i * 4 + 3] = entryNext.getRsi3();
        }
    }
