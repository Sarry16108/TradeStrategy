...
public abstract class AbstractRender {
    ...
    protected TradeInfoSet entrySet; // entry 值
     ...
     * 获取 entry 数据
     */
    public TradeInfoSet getTradeInfoSet() {
        return entrySet;
    }
    ...
    public void zoom(RectF contentRect, float visibleCount, float x, float y) {
        ...
        touchValues[Matrix.MSCALE_X] = entrySet.getTradeInfoList().size() / visibleCount;
        ...
        if (DEBUG) {
            ToolLog.i(TAG, "zoom", "##d zoom: touchValues[Matrix.MSCALE_X] = " + touchValues[Matrix.MSCALE_X]
    ...
    protected void postMatrixValue(float width, float height, float minY, float deltaY) {
        final float scaleX = width / entrySet.getTradeInfoList().size();
    ...
    protected void postMatrixTouch(float width, float visibleCount) {
        final float scaleX = entrySet.getTradeInfoList().size() / visibleCount;
        ...
        if (DEBUG) {
            ToolLog.i(TAG, "postMatrixTouch", "##d postMatrixTouch: currentOffset = " + touchValues[Matrix.MTRANS_X]
        ...
        if (scaleX > 1) {
            if (touchValues[Matrix.MTRANS_X] > 0) {
                ...
                if (overScrollOffset != 0 && !viewRectChange) {
                    ...
                    if (DEBUG) {
                        ToolLog.i(TAG, "postMatrixTouch", "##d postMatrixTouch: transX = " + touchValues[Matrix.MTRANS_X] + ", viewRectChange = " + viewRectChange);
                    }
    ...
    protected float getTransX(float visibleCount, int entryIndex) {
        final int entrySetSize = entrySet.getTradeInfoList().size();
