...
import com.example.finance.tradestrategy.indicators.view.align.XMarkerAlign;
import com.example.finance.tradestrategy.indicators.view.align.YLabelAlign;
...
public class ViewUtils {
     ...
     * 设置 蜡烛图画笔的颜色和是否空心
     */
    public static TradeInfo setUpCandlePaint(Paint candlePaint, TradeInfoSet entrySet, int currentEntryIndex, SizeColor sizeColor) {
        TradeInfo entry = entrySet.getTradeInfoList().get(currentEntryIndex);
        ...
        if (entry.getClose() > entry.getOpen()) { // 今日收盘价大于今日开盘价为涨
            ...
            if (currentEntryIndex > 0) {
                if (entry.getOpen() > entrySet.getTradeInfoList().get(currentEntryIndex - 1).getClose()) { // 今日开盘价大于昨日收盘价为涨停
                    candlePaint.setColor(sizeColor.getIncreasingColor());
                } else if (entry.getOpen() == entrySet.getTradeInfoList().get(currentEntryIndex - 1).getClose()) { // 不涨不跌
