...
public class MACDDrawing implements IDrawing {
    ...
    public void computePoint(int minIndex, int maxIndex, int currentIndex) {
        ...
        final TradeInfo.MACD entry = entrySet.getTradeInfoList().get(currentIndex).getMacd();
        final int i = currentIndex - minIndex;

        if (currentIndex < maxIndex - 1) {
            final TradeInfo.MACD entryNext = entrySet.getTradeInfoList().get(currentIndex + 1).getMacd();
            ...
            deaBuffer[i * 4 + 2] = 0;
            deaBuffer[i * 4 + 3] = entryNext.getDea();

            diffBuffer[i * 4 + 0] = 0;
            diffBuffer[i * 4 + 1] = entry.getDiff();
            diffBuffer[i * 4 + 2] = 0;
            diffBuffer[i * 4 + 3] = entryNext.getDiff();
        }
    }
    ...
    public void onComputeOver(Canvas canvas, int minIndex, int maxIndex, float minY, float maxY) {
        ...
        for (int i = minIndex; i < maxIndex; i++) {
            TradeInfo.MACD entry = entrySet.getTradeInfoList().get(i).getMacd();
