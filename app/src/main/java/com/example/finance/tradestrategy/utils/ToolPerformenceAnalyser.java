...
public class ToolPerformenceAnalyser {
    private static final String TAG = "PerformenceAnalyser";

    private static volatile ToolPerformenceAnalyser instance;
    ...
    private ToolPerformenceAnalyser() {
        analyserCallback = new AnalyserCallback() {
            @Override
            public void onWatcherFinish(String key, int item, long ns, long ms) {
                ToolLog.d(TAG, "ToolPerformenceAnalyser", "##d onWatcherFinish: " + key + "[" + item + "][" + ms + "ms]");
            }
        };
    }

    public static ToolPerformenceAnalyser getInstance() {
        if (instance == null) {
            synchronized (ToolPerformenceAnalyser.class) {
                if (instance == null) {
                    instance = new ToolPerformenceAnalyser();
                }
            }
        }
        return instance;
