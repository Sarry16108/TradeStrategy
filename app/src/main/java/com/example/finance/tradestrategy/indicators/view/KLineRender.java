...
import com.example.finance.tradestrategy.indicators.view.drawing.BOLLDrawing;
import com.example.finance.tradestrategy.indicators.view.drawing.CandleDrawing;
import com.example.finance.tradestrategy.indicators.view.drawing.EmptyDataDrawing;
import com.example.finance.tradestrategy.indicators.view.drawing.HighlightDrawing;
import com.example.finance.tradestrategy.indicators.view.drawing.IDrawing;
import com.example.finance.tradestrategy.indicators.view.drawing.KLineGridAxisDrawing;
import com.example.finance.tradestrategy.indicators.view.drawing.MADrawing;
import com.example.finance.tradestrategy.indicators.view.drawing.TBRegionDrawing;
import com.example.finance.tradestrategy.indicators.view.index.StockIndex;
import com.example.finance.tradestrategy.indicators.view.maker.IMarkerView;
...
public class KLineRender extends AbstractRender {
    ...
    private final TBRegionDrawing   tbRegionDrawing = new TBRegionDrawing();
...
//        kLineDrawingList.add(bollDrawing);
        kLineDrawingList.add(tbRegionDrawing);
    ...
    @Override
    public void setTradeInfoSet(TradeInfoSet entrySet) {
        super.setTradeInfoSet(entrySet);
    ...
    public void zoomIn(float x, float y) {
        if (entrySet.getTradeInfoList().size() == 0) {
            return ;
        }
    ...
    public void render(Canvas canvas) {
        final int count = entrySet.getTradeInfoList().size();

        computeVisibleIndex();

        final float minY = count > 0 ? entrySet.getTradeInfoList().get(entrySet.getMinYIndex()).getLow() : Float.NaN;
        final float maxY = count > 0 ? entrySet.getTradeInfoList().get(entrySet.getMaxYIndex()).getHigh() : Float.NaN;
    ...
    private void computeVisibleIndex() {
        ...
        if (maxVisibleIndex > entrySet.getTradeInfoList().size()) {
            maxVisibleIndex = entrySet.getTradeInfoList().size();
        }

        // 计算当前显示区域内 entry 在 Y 轴上的最小值和最大值
//        entrySet.computeMinMaxWithMa(minVisibleIndex, maxVisibleIndex, stockIndexList);              //MA
        entrySet.computeMinMaxWithBoll(minVisibleIndex, maxVisibleIndex, stockIndexList);       //BOLL
