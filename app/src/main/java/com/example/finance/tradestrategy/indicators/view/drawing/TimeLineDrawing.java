...
public class TimeLineDrawing implements IDrawing {
    ...
    public void computePoint(int minIndex, int maxIndex, int currentIndex) {
        ...
        final TradeInfo entry = entrySet.getTradeInfoList().get(currentIndex);
        ...
        if (currentIndex < maxIndex) {
            ...
            lineBuffer[i * 4 + 3] = entrySet.getTradeInfoList().get(currentIndex + 1).getClose();
        }
    }
    ...
    public void onComputeOver(Canvas canvas, int minIndex, int maxIndex, float minY, float maxY) {
        ...
        if (render.isHighlight()) {
            final TradeInfoSet entrySet = render.getTradeInfoSet();
            final int lastTradeInfoIndex = entrySet.getTradeInfoList().size() - 2;
            ...
            highlightPoint[0] = highlightIndex < lastTradeInfoIndex ?
                    lineBuffer[i * 4 + 0] : lineBuffer[lastTradeInfoIndex * 4 + 2];
            highlightPoint[1] = highlightIndex < lastTradeInfoIndex ?
                    lineBuffer[i * 4 + 1] : lineBuffer[lastTradeInfoIndex * 4 + 3];
        }
